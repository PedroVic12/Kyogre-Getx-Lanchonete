...
class GestaoPedidosController extends GetxController {
  ...
  Future<void> atualizarStatusPedido(int pedidoId, String acao) async {
    final url = Uri.parse(
        'https://rayquaza-citta-server.onrender.com/pedidos/$pedidoId');
    ...
    if (response.statusCode == 200) {
      Get.snackbar("Pedido atualizado com sucesso!", "",
          showProgressIndicator: true, duration: const Duration(seconds: 1));
    } else {
  ...
  void moveRight(Task task) {
    if (task.columnIndex < 3) {
      task.columnIndex++;
      task.pedido.status =
          getIndexStatus(task.columnIndex); // Atualiza o status
      tasks.refresh();
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      child: Container(
        color: color,
        child: Obx(() {
          var columnTasks = kanbanController.tasks
              .where((t) => t.columnIndex == columnIndex)
              .toList();

          return Column(
            children: [
              Text("$titulo - ($columnIndex)",
